...
def get_essay(info,num):
	...
	list1 = soup.select('a[href]')#前三块的识别
...
def get_url_index(url):#handle URL
	website = urllib.urlopen(url)
	html = website.read()
	fp = open("index.txt", 'w')			#创建空文件
	...
	for j in range(0 , target_num_h):
		...
		for k in range(1,target_num_1):
			...
			number = number_1*100000+number_2*1000+k  #code the file 
			get_essay(info,number)
			docid = str(number) + ' '
			fp.write(docid) #record the index
			fp.write(new_url+'\n')#record the url
